...
function LlegadaComprovaLogin(data)
{
	if (data == "1")
	{
		document.location = "http://estabulari.uab.cat/";
	}
	else
	{
...
function HomeCarrega()
{
	...
	$('#MapaWeb').fadeOut(1000);
	$("#DIVHome").delay(1001).fadeIn(1000);
...
{
//	alert("id: "+id+"op: "+op);
	//$("#IdLinMenuActual").val(idLin);
		$("#DIVHome").fadeOut(1000);
		$('#DIVContingutPage').hide();
	
	window.location.hash = id+"|"+op;
...
function LlegadaMostraPageHash(data)
{
	...
	$('#DIVHome').fadeOut(1000);
	$('#MapaWeb').fadeOut(1000);
	$("#ContingutPages").delay(1001).fadeIn(1000);
...
function UpdateUserGU()
{
	...
	if ( $("#PasswordGU").val() ==  $("#Password2GU").val())
	{		
		$.get("PHP/UserGUUpdate.php",{id:id,N:N,A:A,E:E,U:U,P:P,R1:R1,R2:R2,R3:R3,R4:R4},CarregaGridGU);
	}
	else
	{
...
{
	$.get("PHP/TancaSessio.php",{},LlegadaComprovaLogin);
}
...
{ 
	...
	if((FB != 0) && (TE != 0))
	{
		if (FB == 1) var IMG = $("#ImatgeDestacat").val();
