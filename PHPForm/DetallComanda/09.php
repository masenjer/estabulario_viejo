...
function CarregaImpo($id)
{
	...
	$SQL = "SELECT E.NomEspecie_ca, HC.CantM, S.NomSoca, HC.NomSoca as NombreCepa, HC.CantF, HC.Estado, HC.IdImpo
			FROM Impo HC 
			LEFT JOIN  Especie E 
			ON E.IdEspecie = HC.IdEspecie
			LEFT JOIN Soca S  
			ON  S.IdSoca = HC.Soca 
		...
		while ($row = mysql_fetch_array($result))
		{
			...
			$resultado .= '
				<tr>
					...
					<td class="GridLine'.$i.'">'.$row["NomSoca"].''.$row["NombreCepa"].'</td>
