...
';
	...
	<tr class="GridLine'.$i.'">
		...
		<td> a les </td>
		<td>'.$row["Hasta"].'</td>
		<td>'.CarregaSelectReservaFrangHor($row["IdFrangHor"],$row["Estat"]).'</td>
	</tr>
		...
		<td colspan="6">
			<div id="DIVDenegat'.$row["IdFrangHor"].'" '.$display.'>
				<table>
					<tr>
						<td>Motiu</td>
						<td><textarea disabled="disabled" id="TADeniegaFrangHor'.$row["IdFrangHor"].'" style="width:250px" class="fuenteForm">'.Quita($row["Deniega"]).'</textarea></td>
						<td></td>
					</tr>
				</table>
			</div>
...
function CarregaSelectReservaFrangHor($id,$num)
{
	$cadena = array("Pendent d'acceptaci&oacute;","S'accepta", "Es denega"); 
	$color = array("#ffd789","#adff89","#ff8989"); 

	$resultado = '<select disabled="disabled" id="EstatFrangHor'.$id.'" class="fuenteForm" style="background-color:'.$color[$num].'">'; 
