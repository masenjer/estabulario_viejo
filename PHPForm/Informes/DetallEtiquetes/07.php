...
//	echo $SQL;
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		if ($row["MTV"] == 0) $resultado .= '<tr><td><b>No mirar </b>taps vaginals</td></tr>';
		else $resultado .= '
			<tr>
				<td><b>Mirar</b> taps vaginals des del dia '.SelectFecha($row["MTVD"]).' fins al dia '.SelectFecha($row["MTVH"]).'</td>
			</tr>
			';
		if ($row["S"] == 0) $resultado .= '<tr><td><b>No Separar </b></td></tr>';
		else $resultado .= '
			<tr>
				<td><b>Separar</b> el dia '.SelectFecha($row["SF"]).'</td>
			</tr>
			';
	}		
